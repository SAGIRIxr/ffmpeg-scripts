...
if "%MODE%"=="1" (
    ...
    call :detect_lang
    echo 正在封装软字幕（语言: !LANGTITLE!），请稍候...
    ffmpeg -y -i "%VIDEO%" -i "%SRT%" -map 0 -map 1 -c copy -c:s srt -metadata:s:s:0 language=!LANGCODE! -metadata:s:s:0 "title=!LANGTITLE!" "!OUTPUT!"
)
